...
pub fn draw(f: &mut Frame, app: &App) {
    ...
    if size.width < 40 || size.height < 10 {
        f.render_widget(
            Paragraph::new("Terminal too small (min 40×10)").style(Style::new().fg(app.theme_muted())),
            size,
        );
        return;
    }
...
fn draw_status(f: &mut Frame, app: &App, area: Rect) {
    ...
    let text = match app.mode {
        AppMode::Normal =>
            format!(" {} │ ↑↓/jk sel · ←→/hl col · Enter detail · a add · e edit · d del · H/L move · / filter · : cmd · T theme · q quit ", mode_str),
        AppMode::FilterInput =>
    ...
    let line = Line::from(vec![
        ...
        Span::styled(&text[mode_str.len() + 3..], style),
    ]);
    f.render_widget(Paragraph::new(line), area);
}
...
fn draw_detail_overlay(f: &mut Frame, area: Rect, app: &App, card: &Card) {
    ...
    lines.push(Line::from(Span::styled(" Card Detail", Style::new().bold().fg(app.theme_accent()))));
    ...
    if let Some(ref a) = card.assignee {
        lines.push(Line::from(vec![
            ...
            Span::styled(a, Style::new().fg(app.theme_success())),
        ]));
    }
