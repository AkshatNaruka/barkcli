...
> Status: ✅ COMPLETE (verified: web build passes)
...
- [x] **T1.6**: Theme dropdown in App header (replaces ☀/☾ button): Black / Light / System, syncs with system via `matchMedia` listener
...
- [x] **T2.4**: KanbanCard layout v2 — mono muted ID line, title, badges row (priority + labels), footer row (avatar+assignee, due-date chip, checklist ☑ n/m, 💬 count, pin)
- [x] **T2.5**: KanbanColumn header — count badge + total WIP hint; drop highlight uses `--accent` at low opacity
- [x] **T2.6**: App header — breadcrumb `project / <board title>`, card count, view tabs (board/table/calendar/list), theme dropdown, ⌘K button — all token-based
...
> Status: ✅ COMPLETE (verified: cargo build + 22 tests pass, UTF8 tables render)
...
- [x] **T3.9**: Pro commands — `stats.rs` (accent progress bar, green done, red blocked), `sprint.rs` (green ✓ completed), `sync.rs` (green ✓ / red ✗), `license.rs` (green ✓), `version.rs` import
...
> Status: ✅ COMPLETE (verified: cargo build + 22 tests pass)
