...
pub fn run() -> Result<()> {
    ...
    // Clean orphaned sprint metadata for deleted boards
    let sprints_dir = board_dir.join("sprints");
    if sprints_dir.is_dir() {
        let boards = list_board_files()?;
        for entry in std::fs::read_dir(&sprints_dir)
            .context("failed to read sprints directory")?
            .filter_map(|e| e.ok())
        {
            let path = entry.path();
            if let Some(stem) = path.file_stem().and_then(|s| s.to_str()) {
                if !boards.contains(&stem.to_string()) {
                    std::fs::remove_file(&path)
                        .context(format!("failed to remove {}", path.display()))?;
                    println!("Removed orphaned sprints: {}", path.display());
                    cleaned += 1;
                }
            }
        }
    ...
    // Clean orphaned context sidecars for deleted boards
    let context_dir = board_dir.join("context");
            ...
            .filter_map(|e| e.ok())
        {
            ...
            if let Some(stem) = path.file_stem().and_then(|s| s.to_str()) {
                if !boards.contains(&stem.to_string()) {
                    std::fs::remove_file(&path)
                        .context(format!("failed to remove {}", path.display()))?;
                    println!("Removed orphaned context: {}", path.display());
                    cleaned += 1;
                }
            }
        }
