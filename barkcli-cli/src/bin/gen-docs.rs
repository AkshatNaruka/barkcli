    ...
    Command::new("barkcli")
        ...
        .subcommand(
            Command::new("serve")
                ...
                .arg(Arg::new("stop").long("stop").action(ArgAction::SetTrue).help("Stop daemon (SIGTERM, then SIGKILL after 5s)"))
                .arg(Arg::new("kill").long("kill").action(ArgAction::SetTrue).help("Force-kill daemon (SIGKILL) and any process on --port"))
                .arg(Arg::new("force").long("force").action(ArgAction::SetTrue).help("With --stop: SIGKILL immediately and allow killing non-barkcli port occupants"))
        ...
        .subcommand(
            Command::new("listener")
                ...
                .arg(Arg::new("server-url").long("server-url").help("Management server URL").default_value("http://localhost:4321"))
